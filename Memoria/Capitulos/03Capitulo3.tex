...
Para simplificar la subida de los datos al servidor hemos eliminado las tildes de las palabras y reemplazado las "`ñ"' por "`ny"'. Esto no tiene efecto en la interacción con los usuarios ya que ellos introducen las palabras como realmente se escriben y se realiza la transformación antes de buscarlas en el servidor. Al no trabajar con palabras con tildes diacríticas no se producen ambigüedades a la hora de realizar la búsqueda. 
...
Django incluye un servidor web que almacena la base de datos qu contendrá las palabras del diccionario y permitirá hacer las consultas necesarias. La base de datos estará formada por instancias del modelo creado, en nuestro caso cada palabra aparecerá modelada mediante su significante y los grados de certeza que tiene de cada emoción. Para realizar las diferentes consultas sobre las palabras disponibles se crearán una serie de clases que implementan los diferentes métodos de un servicio web REST típico: \textbf{GET, POST, DELETE}. Esencialmente \textbf{GET}, ya que es el que devuelve información sobre la palabra o alguno de sus campos. Cada una de las diferentes clases o "`vistas"' nos aportarán una forma diferente de acceder a la información: acceso a toda la lista, a una palabra concreta, a un campo de una palabra concreta, etc... Los resultados serán devueltos en formato JSON.
